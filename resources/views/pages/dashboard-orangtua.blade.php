...
@include('components.navbar', ['role' => $role])
    ...
    <div class="flex flex-wrap items-center gap-4 mb-8 px-4">
        @forelse($siswa as $s)
            ...
            @endphp
            <a href="{{ route('orangtua.dashboard', ['siswa_id' => $s->id_siswa, 'bulan' => $bulan, 'tahun' => $tahun]) }}" class="inline-block">
        ...
        <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 gap-3">
                <div>
                    ...
                    <p class="text-sm text-gray-500 mt-1">{{ $activeSiswa ? $activeSiswa->nama : 'Pilih siswa untuk melihat data' }}</p>
                </div>
                <form method="GET" action="{{ route('orangtua.dashboard') }}" class="flex flex-wrap items-center gap-3">
                    ...
                    <select name="bulan" class="border border-gray-300 rounded-lg px-3 py-2 text-sm bg-white">
                        @foreach (range(1, 12) as $m)
                            <option value="{{ $m }}" {{ $bulan == $m ? 'selected' : '' }}>{{ Carbon::createFromDate($tahun, $m, 1)->translatedFormat('F') }}</option>
                        @endforeach
                    </select>
                    <button type="submit" class="bg-[#313589] text-white rounded-lg px-3 py-2 text-sm">Terapkan</button>
                </form>
            </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 px-4 mb-8">
        ...
        <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
            ...
            @if ($latestDokumentasi)
                ...
                @endphp
                <div class="mb-6">
                    <img src="{{ $imageUrl }}" alt="Dokumentasi terbaru" class="w-full rounded-xl object-cover h-40">
                </div>
                <div>
                    <p class="text-xs uppercase tracking-[0.2em] text-slate-500 mb-2">{{ Carbon::parse($latestDokumentasi->tanggal)->translatedFormat('d F Y') }}</p>
                    <p class="font-semibold text-gray-900 mb-2">{{ $latestDokumentasi->judul }}</p>
                    <p class="text-gray-600 text-sm leading-relaxed mb-4">{{ Str::limit($latestDokumentasi->deskripsi, 140) }}</p>
                    <div class="flex flex-wrap items-center gap-2">
                        <span class="text-xs bg-slate-100 text-slate-700 px-2 py-1 rounded-full">{{ $latestDokumentasi->kelas?->nama_kelas ?? 'Semua Kelas' }}</span>
                        @if($latestDokumentasi->guru)
                            <span class="text-xs bg-blue-50 text-blue-700 px-2 py-1 rounded-full">Guru: {{ $latestDokumentasi->guru->name }}</span>
                        @endif
                    </div>
                </div>
                <a href="{{ route('orangtua.dokumentasi.detail', $latestDokumentasi->id_kegiatan) }}" class="text-blue-500 text-sm font-medium mt-4 inline-block hover:underline">Lihat Detail</a>
            @else
                <div class="text-gray-500">
                    <p class="font-semibold mb-3">Tidak ada dokumentasi terbaru.</p>
                    <p class="text-sm">Saat ini belum ada dokumentasi yang sesuai dengan anak Anda.</p>
                </div>
            @endif
        </div>
        ...
        if (ctx) {
            new Chart(ctx, {
                ...
                data: {
                    ...
                    datasets: [{
                        data: [{{ $absensiChart[0] }}, {{ $absensiChart[1] }}, {{ $absensiChart[2] }}, {{ $absensiChart[3] }}],
                        backgroundColor: [
                            '#3B82F6',
                            '#F97316',
                            '#FCD34D',
