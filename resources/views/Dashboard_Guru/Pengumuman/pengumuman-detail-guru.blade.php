...
@php
    ...
    $currentKelasId = $kelas ? $kelas->id_kelas : null;
    $role = 'guru';
    $isOwnClass = $pengumuman->kelas_id == $currentKelasId;
@endphp
...
<div class="mb-8">
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 px-4 py-2 pt-4">
        <div>
            ...
            <p class="text-gray-600 mt-1">Detail Pengumuman</p>
        </div>
...
<div class="bg-white rounded-2xl p-8 shadow-sm border border-gray-200 max-w-4xl">
    ...
    <div class="flex flex-col gap-1 mb-6">
        ...
        @if ($pengumuman->kelas)
            <p class="text-gray-700 text-sm">
                ...
                @if(!$isOwnClass)
                    <span class="ml-2 text-xs bg-gray-100 text-gray-600 px-2 py-1 rounded">Publik</span>
                @endif
            </p>
        @else
            <p class="text-gray-700 text-sm">
    ...
    <div class="mt-8 flex flex-wrap gap-3 pt-6 border-t border-gray-100">
        <a href="{{ route('guru.pengumuman.edit', $pengumuman->id_pengumuman) }}"
