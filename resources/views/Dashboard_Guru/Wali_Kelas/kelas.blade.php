...
<div id="pageContent" class="transition duration-300">
    <div class="mb-8">
        <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div>
                ...
                <p class="text-sm text-slate-500">Kelola siswa di kelas Anda</p>
            </div>
        </div>
    </div>
    ...
    @if (session('success'))
        <div class="mb-6 rounded-[24px] border border-green-200 bg-green-50 p-5 text-sm text-green-700">
            {{ session('success') }}
        </div>
    @endif
                    ...
                    <table class="w-full">
                        <thead>
                            <tr class="border-b border-slate-200 bg-slate-50">
                                <th class="px-6 py-4 text-left text-xs font-semibold text-slate-700">#</th>
                                <th class="px-6 py-4 text-left text-xs font-semibold text-slate-700">Nama</th>
                                <th class="px-6 py-4 text-left text-xs font-semibold text-slate-700">NIS</th>
                                <th class="px-6 py-4 text-left text-xs font-semibold text-slate-700">NISN</th>
                                <th class="px-6 py-4 text-left text-xs font-semibold text-slate-700">Jenis Kelamin</th>
                                <th class="px-6 py-4 text-left text-xs font-semibold text-slate-700">Alamat</th>
                                <th class="px-6 py-4 text-left text-xs font-semibold text-slate-700">Aksi</th>
                            </tr>
                        </thead>
                                    ...
                                    <td class="px-6 py-4 text-sm">
                                        <div class="flex items-center gap-2">
                                            <button
                                                ...
                                                title="Lihat Detail"
                                                class="inline-flex items-center justify-center rounded-[10px] bg-blue-500 p-2 text-white transition hover:bg-blue-600"
                                            >
                                            ...
                                            <a
                                                ...
                                                title="Edit"
                                                class="inline-flex items-center justify-center rounded-[10px] bg-emerald-500 p-2 text-white transition hover:bg-emerald-600"
                                            >
                                                <svg xmlns="http://www.w3.org/2000/svg"
                                                    fill="none"
                                                    viewBox="0 0 24 24"
...
<div
    id="detailModal"
    class="fixed inset-0 z-50 hidden items-center justify-center bg-black/20 backdrop-blur-sm p-4"
>
    <div class="w-full max-w-2xl rounded-[32px] bg-white p-8 shadow-2xl">
        ...
        <div class="mt-6 flex justify-end">
            <button
                type="button"
                onclick="closeDetailModal()"
                class="rounded-[16px] border border-slate-200 bg-slate-50 px-5 py-2 text-sm font-semibold text-slate-700 transition hover:bg-slate-100"
            >
                Tutup
            </button>
        </div>
        ...
        fetch(`/guru/siswa/${siswaId}`)
            ...
            .then(data => {
                ...
                const orangTuaHtml = data.orang_tua
                    ...
                    `
                    : `
                        <div class="rounded-[16px] border border-dashed border-slate-300 p-4 text-sm text-slate-500">
                            Tidak ada data orang tua / wali
                        </div>
                    `;
            ...
            .catch(error => {
                content.innerHTML = `
                    <div class="rounded-[16px] bg-red-50 p-4 text-sm text-red-600">
