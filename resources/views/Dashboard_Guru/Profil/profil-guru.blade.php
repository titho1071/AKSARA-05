                    ...
                    <div x-show="activeTab === 'edit-foto'" style="display: none;">
                        <form action="{{ route('guru.profil.foto') }}" method="POST" enctype="multipart/form-data" class="text-center">
                            ...
                            <div class="w-32 h-32 mx-auto mb-8 relative group">
                                <img id="preview-image" src="{{ $guru->foto_profil ? asset('storage/' . $guru->foto_profil) : '' }}" alt="Preview" class="w-full h-full rounded-full object-cover {{ $guru->foto_profil ? '' : 'hidden' }}">
                                
                                <div id="preview-placeholder" class="w-full h-full rounded-full bg-gray-100 flex items-center justify-center {{ $guru->foto_profil ? 'hidden' : '' }}">
