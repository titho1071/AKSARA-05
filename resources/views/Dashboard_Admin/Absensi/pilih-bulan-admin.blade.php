    ...
    <div class="max-w-[1400px] mx-auto space-y-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            @foreach ($months as $month)
                <a href="{{ route('admin.absensi.detail', ['id' => $kelas->id_kelas, 'bulan' => strtolower(explode(' ', $month['name'])[0])]) }}" class="group bg-white rounded-[16px] border border-slate-200 p-5 flex items-center gap-4 transition-all duration-200 shadow-sm hover:shadow-md {{ $month['card_hover'] }}">
                    <!-- Calendar Icon -->
