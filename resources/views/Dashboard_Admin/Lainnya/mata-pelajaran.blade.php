...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function renderTable() {
            ...
            tableBody.innerHTML = mapelData.map((item, index) => `
                <tr class="hover:bg-slate-50 transition-colors">
                    ...
                    <td class="px-6 py-4 text-slate-700">${item.nama}</td>
                    <td class="px-6 py-4">
        ...
        function openModal(mode, item = null) {
            ...
            modalDesc.textContent = mode === 'edit' ? 'Perbarui nama mata pelajaran yang dipilih.' : 'Isi nama mata pelajaran baru.';
        ...
        function saveMapel() {
            ...
            const duplicate = mapelData.find(m => m.nama.toLowerCase() === nama.toLowerCase() && m.id !== editingId);
            if (duplicate) {
                showError('Mata pelajaran dengan nama ini sudah ada.');
                return;
            }

            if (editingId) {
                const index = mapelData.findIndex(item => item.id === editingId);
                if (index !== -1) {
                    mapelData[index].nama = nama;
                }
            } else {
                const nextId = mapelData.length ? Math.max(...mapelData.map(item => item.id)) + 1 : 1;
                mapelData.push({ id: nextId, nama });
            }
        ...
        tableBody.addEventListener('click', function (event) {
            ...
            if (action === 'delete' && item) {
                if (confirm(`Hapus mata pelajaran "${item.nama}"?`)) {
