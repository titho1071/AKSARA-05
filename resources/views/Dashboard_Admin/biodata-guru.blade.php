...
@section('content')
    ...
    <div class="mb-8 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
            <p class="text-sm font-semibold text-slate-500">Biodata Guru</p>
            <h1 class="text-3xl font-bold text-slate-950">Kelola Biodata Guru</h1>
            <p class="text-sm text-slate-500">Lihat dan tambahkan data guru sekolah.</p>
        </div>
                ...
                <tbody class="divide-y divide-slate-200">
                    @forelse ($gurus as $index => $guru)
                        <tr class="odd:bg-slate-50 even:bg-white">
                            ...
                            <td class="px-4 py-4 text-slate-700">{{ $guru->gender ?? '-' }}</td>
    ...
    <div id="deleteModal" class="fixed inset-0 z-50 hidden items-center justify-center">
        ...
        <div class="relative w-full max-w-sm rounded-[28px] bg-white p-8 shadow-xl">
            ...
            <h2 class="mb-2 text-center text-lg font-bold text-slate-900">Hapus Guru</h2>
            <p class="mb-1 text-center text-sm text-slate-500">Apakah kamu yakin ingin menghapus</p>
            <p id="deleteGuruName" class="mb-6 text-center text-sm font-semibold text-slate-800"></p>
    ...
    <script>
        function confirmDelete(actionUrl, namaGuru) {
            ...
            document.getElementById('deleteGuruName').textContent = namaGuru;
