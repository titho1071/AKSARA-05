            ...
            <div class="bg-white rounded-3xl p-8 shadow-sm border border-gray-100 text-center">
                <div class="relative w-32 h-32 mx-auto mb-6">
                    @if($admin->foto_profil)
                        <img src="{{ asset('storage/' . $admin->foto_profil) }}" alt="Foto Profil" class="w-full h-full rounded-full object-cover border-4 border-white shadow-md">
                    @else
                ...
                <h2 class="text-2xl font-bold text-gray-900">{{ $admin->nama ?? 'Admin' }}</h2>
                <p class="text-gray-500 mb-8">Administrator</p>
            ...
            <div class="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
                ...
                <div class="p-8">
                        ...
                        <form action="{{ route('admin.profil.update') }}" method="POST" class="space-y-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-4 items-center gap-4">
                                ...
                                <div class="md:col-span-3">
                                    <input type="text" name="nama" value="{{ old('nama', $admin->nama) }}" class="w-full rounded-xl border border-gray-200 px-4 py-3 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none transition-all">
                                </div>
                            </div>
                            <div class="grid grid-cols-1 md:grid-cols-4 items-center gap-4">
                                <label class="md:col-span-1 font-bold text-gray-700">NIP</label>
                                <div class="md:col-span-3">
                                    <input type="text" name="nip" value="{{ old('nip', $admin->nip) }}" class="w-full rounded-xl border border-gray-200 px-4 py-3 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none transition-all">
                                </div>
                            </div>
                            <div class="grid grid-cols-1 md:grid-cols-4 items-center gap-4">
                                <label class="md:col-span-1 font-bold text-gray-700">NUPTK</label>
                                <div class="md:col-span-3">
                                    <input type="text" name="nuptk" value="{{ old('nuptk', $admin->nuptk) }}" class="w-full rounded-xl border border-gray-200 px-4 py-3 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none transition-all">
                                </div>
                            </div>
                            <div class="grid grid-cols-1 md:grid-cols-4 items-center gap-4">
                                ...
                                <div class="md:col-span-3">
                                    <select name="gender" class="w-full rounded-xl border border-gray-200 px-4 py-3 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none transition-all appearance-none bg-no-repeat bg-[right_1rem_center] bg-[length:1em_1em]" style="background-image: url('data:image/svg+xml,%3Csvg xmlns=%22http://www.w3.org/2000/svg%22 fill=%22none%22 viewBox=%220 0 24 24%22 stroke=%22%236b7280%22%3E%3Cpath stroke-linecap=%22round%22 stroke-linejoin=%22round%22 stroke-width=%222%22 d=%22M19 9l-7 7-7-7%22 /%3E%3C/svg%3E');">
                                        <option value="Laki-laki" {{ old('gender', $admin->jenis_kelamin) == 'Laki-laki' ? 'selected' : '' }}>Laki-laki</option>
                                        <option value="Perempuan" {{ old('gender', $admin->jenis_kelamin) == 'Perempuan' ? 'selected' : '' }}>Perempuan</option>
                                    </select>
                                </div>
                            </div>
                            <div class="grid grid-cols-1 md:grid-cols-4 items-center gap-4">
                                ...
                                <div class="md:col-span-3">
                                    <input type="text" name="phone" value="{{ old('phone', $admin->no_hp) }}" class="w-full rounded-xl border border-gray-200 px-4 py-3 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none transition-all">
                                </div>
                            </div>
                            <div class="grid grid-cols-1 md:grid-cols-4 items-start gap-4">
                                ...
                                <div class="md:col-span-3">
                                    <textarea name="address" rows="4" class="w-full rounded-xl border border-gray-200 px-4 py-3 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none transition-all">{{ old('address', $admin->alamat) }}</textarea>
                                </div>
                            </div>
                    ...
                    <div x-show="activeTab === 'edit-foto'" style="display: none;">
                        <form action="{{ route('admin.profil.foto') }}" method="POST" enctype="multipart/form-data" class="text-center">
                            @csrf
                            @method('PUT')
                            <h3 class="text-lg font-bold mb-6">Foto</h3>
                            <div class="w-32 h-32 mx-auto mb-8 relative group">
                                <img id="preview-image" src="{{ $admin->foto_profil ? asset('storage/' . $admin->foto_profil) : '' }}" alt="Preview" class="w-full h-full rounded-full object-cover {{ $admin->foto_profil ? '' : 'hidden' }}">
                                
                                <div id="preview-placeholder" class="w-full h-full rounded-full bg-gray-100 flex items-center justify-center {{ $admin->foto_profil ? 'hidden' : '' }}">
                    ...
                    <div x-show="activeTab === 'edit-akun'" style="display: none;">
                        <form id="form-edit-akun" action="{{ route('admin.profil.akun') }}" method="POST" class="space-y-6">
                            @csrf
                            @method('PUT')
                            <div class="grid grid-cols-1 md:grid-cols-4 items-center gap-4">
