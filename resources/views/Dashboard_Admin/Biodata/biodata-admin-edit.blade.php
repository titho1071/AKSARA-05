...
<div class="mb-8">
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 px-4 py-2 pt-4">
        <div>
            <h1 class="text-3xl md:text-4xl font-bold text-gray-900">Edit Data Admin</h1>
            <p class="text-gray-600 mt-1">Perbarui informasi data admin.</p>
        </div>
...
<div class="rounded-[32px] border border-slate-200 bg-white p-8 shadow-sm">
    <form id="biodata-form" action="{{ route('admin.biodata.update', $user->id) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        <div class="grid gap-6 lg:grid-cols-2">
            <div>
                <label class="mb-2 block text-sm font-semibold text-slate-700">Nama Lengkap</label>
                <input type="text" name="nama" value="{{ old('nama', $profil->nama) }}" placeholder="Masukkan nama lengkap"
                    class="w-full rounded-[16px] border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-700 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-100" required />
            </div>
            <div>
                <label class="mb-2 block text-sm font-semibold text-slate-700">NIP</label>
                <input type="text" name="nip" value="{{ old('nip', $profil->nip) }}" placeholder="Masukkan NIP"
            ...
            <div>
                ...
                <input type="text" name="nuptk" value="{{ old('nuptk', $profil->nuptk) }}" placeholder="Masukkan NUPTK"
                    class="w-full rounded-[16px] border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-700 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-100" />
            </div>
            <div>
                ...
                <select name="gender" class="w-full rounded-[16px] border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-700 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-100">
                    ...
                    <option value="Laki-laki" {{ old('gender', $profil->jenis_kelamin) === 'Laki-laki' ? 'selected' : '' }}>Laki-laki</option>
                    <option value="Perempuan" {{ old('gender', $profil->jenis_kelamin) === 'Perempuan' ? 'selected' : '' }}>Perempuan</option>
                </select>
            </div>
            <div>
                <label class="mb-2 block text-sm font-semibold text-slate-700">Telepon</label>
                <input type="text" name="phone" value="{{ old('phone', $profil->no_hp) }}" placeholder="Masukkan telepon"
                    class="w-full rounded-[16px] border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-700 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-100" />
            </div>
            <div>
                <label class="mb-2 block text-sm font-semibold text-slate-700">Alamat</label>
                <input type="text" name="address" value="{{ old('address', $profil->alamat) }}" placeholder="Masukkan alamat"
                    class="w-full rounded-[16px] border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-700 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-100" />
            </div>
            <div>
                ...
                <select name="status" class="w-full rounded-[16px] border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-700 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-100">
                    ...
                    <option value="tidak_aktif" {{ old('status', $profil->status) === 'tidak_aktif' ? 'selected' : '' }}>Tidak Aktif</option>
                </select>
            </div>
            <div>
            ...
            <div>
                <label class="mb-2 block text-sm font-semibold text-slate-700">Password Baru <span class="text-slate-400 font-normal">(kosongkan jika tidak diubah)</span></label>
                <div class="relative">
                    <input type="password" id="password" name="password" placeholder="Masukkan password baru"
                        class="w-full rounded-[16px] border border-slate-200 bg-slate-50 px-4 py-3 pr-12 text-sm text-slate-700 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-100" />
