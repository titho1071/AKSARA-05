...
class KelasController extends Controller
{
    public function index()
    {
        try {
            $kelas = DB::table('kelas')
                ...
                ->select('kelas.*', 'guru.nama as guru_nama', 'tahun_pelajaran.tahun_pelajaran as tapel_nama')
                ->get();
    ...
    public function store(Request $request)
    {
        ...
        if ($exists) {
            return response()->json([
                'message' => 'Kelas dengan nama "' . $request->nama_kelas . '" sudah ada.'
            ], 422);
        }
        ...
        if ($request->guru_id) {
            ...
            if ($guruKelas) {
                ...
                return response()->json([
                    'message' => 'Guru "' . $namaGuru . '" sudah menjadi wali kelas "' . $guruKelas->nama_kelas . '".'
                ], 422);
            }
        }
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($request->guru_id) {
            $guruKelas = Kelas::where('guru_id', $request->guru_id)->where('id_kelas', '!=', $id)->first();
            if ($guruKelas) {
                $namaGuru = DB::table('guru')->where('id_guru', $request->guru_id)->value('nama');
                return response()->json([
                    'message' => 'Guru "' . $namaGuru . '" sudah menjadi wali kelas "' . $guruKelas->nama_kelas . '".'
